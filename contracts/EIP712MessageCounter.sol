...
contract EIP712MessageCounter is EIP712 {
    mapping(address => uint256) private _counters; // por cada mensaje off_chain
    mapping(address => string) private _accountsLastMessage; // ultimo mensaje

    struct Signature {
        address signer; // direccion que hace la firma off_chain
        string message; // se escribe un mensaje
    }
